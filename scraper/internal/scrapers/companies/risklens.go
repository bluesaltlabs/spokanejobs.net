...
type RisklensScraper struct {
	*types.BaseScraper
}

func NewRisklensScraper() *RisklensScraper {
	return &RisklensScraper{
		BaseScraper: &types.BaseScraper{
			Name:         "risklens",
			ScheduleHour: 18,
			Config: types.ScraperConfig{
				BaseURL: "https://www.risklens.com/careers/",
			},
		},
	}
}

func (scraper *RisklensScraper) scrapeJobs() []types.ScrapedJob {
...
func (scraper *RisklensScraper) GetName() string {
	return scraper.Name
}

func (scraper *RisklensScraper) ScrapedJobs() []types.ScrapedJob {
...
func (scraper *RisklensScraper) ScrapeJobDetails(job *types.ScrapedJob) {
	// Default implementation - can be overridden if needed
}

func (scraper *RisklensScraper) SaveOutput(outputDir string) error {
