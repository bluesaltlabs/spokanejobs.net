...
type TwoBarrelsScraper struct {
	*types.BaseScraper
}

func NewTwoBarrelsScraper() *TwoBarrelsScraper {
	return &TwoBarrelsScraper{
		BaseScraper: &types.BaseScraper{
			Name:         "two_barrels",
			ScheduleHour: 3,
			Config: types.ScraperConfig{
				BaseURL: "https://2barrels.com/careers",
			},
		},
	}
}

func (scraper *TwoBarrelsScraper) GetName() string {
	return scraper.Name
}
...
func (scraper *TwoBarrelsScraper) ScrapeJobDetails(job *types.ScrapedJob) {
	// Default implementation - can be overridden if needed
}

func (scraper *TwoBarrelsScraper) SaveOutput(outputDir string) error {
